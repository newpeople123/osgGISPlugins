...
void createModel23dtilesToolTab(bool enable) {
    ...
    if (ImGui::BeginTabItem("model23dtiles"))
    {
        ...
        if (ImGui::CollapsingHeader("输入输出参数", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::BeginChild("IOGroup", ImVec2(0, 130), true);
            {
                ...
                if (ImGui::Button("选择文件")) {
                    const char* filters[] = { "*.fbx", "*.obj", "*.3ds", "*.dxf", "*.ive", "*.osg", "*.osgb" };
                    ...
                    if (outPath) {
                        strncpy(modelParams.inputModel, outPath, IM_ARRAYSIZE(modelParams.inputModel));
                        modelParams.inputModel[IM_ARRAYSIZE(modelParams.inputModel) - 1] = '\0';
                    }
                }
                ...
                ImGui::SameLine();
                if (ImGui::Button("选择输出文件夹")) {
                    const char* outPath = tinyfd_selectFolderDialog("选择输出文件夹", modelParams.outputDir);
                    if (outPath) {
                        strncpy(modelParams.outputDir, outPath, IM_ARRAYSIZE(modelParams.outputDir));
                        modelParams.outputDir[IM_ARRAYSIZE(modelParams.outputDir) - 1] = '\0';
                        //modelParams.outputDir = osgDB::convertStringFromUTF8toCurrentCodePage(outPath);
                    }
                }
            }
            ImGui::EndChild();
        ...
        // 材质参数
        ImGui::Separator();
        if (ImGui::CollapsingHeader("材质参数", ImGuiTreeNodeFlags_CollapsingHeader)) {
            ImGui::Indent();
            ImGui::BeginChild("MaterialGroup", ImVec2(0, 240), true);
            {
                double minVal = 0.01, maxVal = 1.0;
                ImGui::Text("透明模式 (-alphamode)");
                ImGui::Combo("##alphaMode", &modelParams.alphaModeIndex,
                    modelParams.alphaModes, IM_ARRAYSIZE(modelParams.alphaModes));
                ImGui::SameLine();
                showHelpMarker("glTF透明模式：OPAQUE/MASK/BLEND/从模型中读取");

                double minCut = 0.0, maxCut = 1.0;
                ImGui::Text("Alpha裁剪阈值 (-alphacutoff)");
                ImGui::DragScalar("##alphaCutoff", ImGuiDataType_Double,
                    &modelParams.alphaCutoff, 0.01, &minCut, &maxCut);
                ImGui::SameLine();
                showHelpMarker("仅在 alphaMode = MASK 时生效。默认 0.5。");


                ImGui::Text("双面渲染 (doubleside)");
                ImGui::Checkbox("##doubleSided", &modelParams.doubleSide);
                ImGui::SameLine();
                showHelpMarker("启用后不会背面剔除。默认 false。");
            }
            ImGui::EndChild();
            ImGui::Unindent();
        }
        ...
        if (ImGui::Button("转换", ImVec2(150, 40))) {
            ...
            if (modelParams.alphaModeIndex > -1) {
                args << " -alphamode " << modelParams.alphaModes[modelParams.alphaModeIndex];
            }
            args << " -alphacutoff " << modelParams.alphaCutoff;
        ...
        if (ImGui::CollapsingHeader("输入输出参数", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::BeginChild("b3dmIO", ImVec2(0, 130), true);
            {
                ...
                if (ImGui::Button("选择输入文件")) {
                    ...
                    if (outPath) {
                        strncpy(inputB3dm, outPath, IM_ARRAYSIZE(inputB3dm));
                        inputB3dm[IM_ARRAYSIZE(inputB3dm) - 1] = '\0';
                    }
                }
                ...
                ImGui::SameLine();
                if (ImGui::Button("选择输出文件夹")) {
                    const char* outPath = tinyfd_selectFolderDialog("选择输出文件夹", outputFile);
                    if (outPath) {
                        strncpy(outputFile, outPath, IM_ARRAYSIZE(outputFile));
                        outputFile[IM_ARRAYSIZE(outputFile) - 1] = '\0';
                    }
                }
            }
            ImGui::EndChild();
