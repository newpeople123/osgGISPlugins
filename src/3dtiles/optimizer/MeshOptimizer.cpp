	...
	for (size_t primIndex = 0; primIndex < psetCount; ++primIndex) {
		...
		if (pset->getMode() == osg::PrimitiveSet::Mode::TRIANGLES) {
			...
			if (type == osg::PrimitiveSet::DrawElementsUBytePrimitiveType) {
				...
				osg::ref_ptr<osg::UByteArray> optimizedIndices = _meshSimplifier->reindexPrimitiveSet<osg::DrawElementsUByte, osg::UByteArray>(&geometry, drawElements, primIndex, remap);
				...
				osg::ref_ptr<osg::UShortArray> optimizedIndices = _meshSimplifier->reindexPrimitiveSet<osg::DrawElementsUShort, osg::UShortArray>(&geometry, drawElements, primIndex, remap);
				if (!optimizedIndices) continue;
				...
				osg::ref_ptr<osg::UIntArray> optimizedIndices = _meshSimplifier->reindexPrimitiveSet<osg::DrawElementsUInt, osg::UIntArray>(&geometry, drawElements, primIndex, remap);
				if (!optimizedIndices) continue;
