...
size_t TexturePacker::getId(osg::Image* image) const
{
	for (const auto& entry : _input)
	{
		if (entry.second.first.get() == image ||
			...
			if (entry.second.first->getFileName().empty()) {
				entry.second.first->setFileName(computeImageHash(entry.second.first.get()));
			}
			if (image->getFileName().empty()) {
				image->setFileName(computeImageHash(image));
			}
			if (entry.second.first->getFileName() == image->getFileName())
				return entry.first;
		}
	}
	return 0;
...
std::string TexturePacker::computeImageHash(const osg::ref_ptr<osg::Image>& image)
{
	size_t hash = 0;
