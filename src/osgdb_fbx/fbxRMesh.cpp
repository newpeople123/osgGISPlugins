...
void fbxProperty2OsgUserValue(const FbxNode* pNode, osg::Node& node) {
    if (pNode) {

        if (pNode) {
            FbxProperty prop = pNode->GetNextProperty(pNode->GetFirstProperty());
            ...
            while (prop.IsValid())
            {
                if (prop.GetFlag(FbxPropertyFlags::eUserDefined)) {
                    ...
                    case fbxsdk::eFbxChar:
                        node.setUserValue(key, prop.Get<char>());
                        break;
                    case fbxsdk::eFbxUChar:
                        node.setUserValue(key, prop.Get<unsigned char>());
                        break;
                    case fbxsdk::eFbxShort:
                        node.setUserValue(key, prop.Get<short>());
                        break;
                    case fbxsdk::eFbxUShort:
                        node.setUserValue(key, prop.Get<unsigned short>());
                        break;
                    case fbxsdk::eFbxUInt:
                        node.setUserValue(key, prop.Get<unsigned int>());
                        break;
                    case fbxsdk::eFbxLongLong:
                        //node.setUserValue(key, prop.Get<long long>());
                        break;
                    case fbxsdk::eFbxULongLong:
                        //node.setUserValue(key, prop.Get<unsigned long long>());
                        break;
                    case fbxsdk::eFbxHalfFloat:
                        break;
                    case fbxsdk::eFbxBool:
                        node.setUserValue(key, prop.Get<bool>());
                        break;
                    case fbxsdk::eFbxInt:
                        node.setUserValue(key, prop.Get<int>());
                        break;
                    case fbxsdk::eFbxFloat:
                        node.setUserValue(key, prop.Get<float>());
                        break;
                    case fbxsdk::eFbxDouble:
                        node.setUserValue(key, prop.Get<double>());
                        break;
                    case fbxsdk::eFbxDouble2:
                        value2 = prop.Get<FbxDouble2>();
                        val2.set(value2[0], value2[1]);
                        node.setUserValue(key, val3);
                        break;
                    case fbxsdk::eFbxDouble3:
                        value3 = prop.Get<FbxDouble3>();
                        val3.set(value3[0], value3[1], value3[2]);
                        node.setUserValue(key, val3);
                        break;
                    case fbxsdk::eFbxDouble4:
                        value4 = prop.Get<FbxDouble4>();
                        val4.set(value4[0], value4[1], value4[2], value4[3]);
                        node.setUserValue(key, val4);
                        break;
    ...
    const char* szName)
{
    ...
    fbxProperty2OsgUserValue(pNode, *pGeode);
