...
protected:
	osg::ref_ptr<TileNode> buildTree(const osg::BoundingBox& total, const osg::ref_ptr<osg::Group>& inputRoot, int x = 0, int y = 0, int z = 0, const osg::ref_ptr<TileNode>& parent = nullptr, int depth = 0) {
		if (total.valid()) {
			int s[3];
...
inline OctreeBuilder::~OctreeBuilder()
{}
